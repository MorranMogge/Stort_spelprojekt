...
class GUIText final : public GUIObject
{
public:

	GUIText();
	GUIText(const float x, const float y, const float layer = 0.0f);
	GUIText(const DirectX::SimpleMath::Vector2& position, const float layer = 0.0f);
